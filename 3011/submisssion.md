...
```text
โจทย์กำหนดมาให้มีแม่สี 3สี แล้วให้เอาแม่สีแต่ละตัวมาผสมกันตามโจทย์สั่ง ในแต่ละรอบที่ผสมสีกันนั้นจะใช้แค่ 2สีเลยได้inputมาเป็นcolo1 and 2 และถ้าผสมสีไก้ตามที่โจทย์ต้องการก้กำหนด outputตามโจทย์ และถ้าหาก สีที่ผสมไม่ได้อยู่ตามแม่สีที่โจทย์กำหนดจะได้ output เป็น error ที่สำคัญคืออยาลืมว่าสีที่มันผสมด้วยกันเองจะได้สีเดิม 
```
...
```text
Step 1:ตั้ง input มา2ตัว
Step 2:กำหนดcolor1 == color2 เพราะ สีที่มันผสมด้วยกันเองต้องได้สีเดิม 
Step 3:ใช้if else elif 
```
...
```text
เริ่มจากการตั้งinputขึ้นมาโจทย์กำหนดมาว่าให้ผสมกัน2สีจึงมีinput 2 ตัวแล้วก็ใช้ if elif else มาใช้ เริ่มจากการใช้ if อย่าลืมว่าสีที่ผสมกันเองต้องได้สีเดิม และสั่งปริ้น ต่อด้วยใช้elif เป็นcolor1==สีที่โจทย์กำหนด และย่อยลงมาใช้ifเหมือนเดิมแต่ใช้เป็นสีที่2ที่โจทย์กำหนด และสั่งปริ้น ต่อมาใช้elif ใช้คำสั่งแบบเดิม อย่าลืมที่ว่าสี1 และสี2 สามารถสลับกันได้ สุดท้ายคือelse คือใช้ในกีณีอื่นๆที่ไม่มีสีี่ผสมอยู่ในแม่สีสั่งปริ้นerror
```
